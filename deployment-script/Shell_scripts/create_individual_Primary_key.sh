...
rm -rf Primary_key_error_log.log

ERROR_LOG=Primary_key_error_log.log

myfolders[0]="Primary_key"



now=$(date +"%T")
echo "Primary_key creation STARTED : $now"
echo "Primary_key creation STARTED : $now" >> $ERROR_LOG 2>&1
...
now=$(date +"%T")
echo "Primary_key creation END : $now"
echo "Primary_key creation END : $now" >> $ERROR_LOG 2>&1
