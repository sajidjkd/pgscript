...
ERROR_LOG = configParser.get('PARAMETERS', 'OC_ERROR_LOG')
...
def create_schema(schemaFlag, password):
    ...
    schema_objects = configParser.get('PARAMETERS', 'schema_objects').split(',')
    queries_list = []
    num_queries = 0
    ...
    if schemaFlag == 'A':
        ...
        sqlres = "select schema_name from information_schema.schemata where schema_name not in ('pg_catalog','information_schema','public','testschema')  order by schema_name "
